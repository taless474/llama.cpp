...
int main(int argc, char ** argv) {
    ...
    // empty. submit_request carries its own per-request prompt vector
    // below.
    opts.preload.prompt_tokens    = nullptr;
    opts.preload.budgets          = {};
    // M4b: exercise the null waiting_queue path together with the
    // per-request streaming path. submit_request(want_stream=true)
    // must work without preloaded waiting requests, stream_all, or
