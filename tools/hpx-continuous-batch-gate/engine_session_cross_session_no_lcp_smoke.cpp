// engine_session_cross_session_no_lcp_smoke.cpp — B+1 guard that LCP
// reuse is SAME-SESSION ONLY: a different session_id must not reuse
// another session's resident prefix, even when the token prefix is
// identical and well above LCP_MIN_TOKENS.
//
// Shape (single engine, n_seq_max=2, keep_alive, greedy):
//   rA       : session "A", SHARED + alpha -> resident (slot)
//   baseline : no session,  SHARED + beta  -> full prefill reference
//   rB       : session "B", SHARED + beta  -> SHARED is an identical long
//              prefix of rA's resident, but B != A, so B+1 must NOT reuse;
//              full prefill on a free slot.
//
// Asserts: lcp_reuse_admitted_count == 0 and session_reuse_admitted_count
// == 0 (no cross-session reuse of any kind), rB output == baseline,
// teardown clean.
...
void print_usage(const char * argv0) {
    fprintf(stderr, "usage: %s --model <path>\n"
        "  B+1 cross-session no-LCP guard.\n", argv0);
}
...
void emit_fail(const std::string & reason) {
    fprintf(stdout, "HPX_SESSION_CROSS_SESSION_NO_LCP_SMOKE: FAIL: %s\n",
            reason.c_str());
    fflush(stdout);
}
...
int main(int argc, char ** argv) {
    ...
    std::vector<llama_token> p_alpha = common_tokenize(
        ctx, std::string(SHARED) + " The old topic is alpha.", true, true);
    std::vector<llama_token> p_beta = common_tokenize(
        ctx, std::string(SHARED) + " The new topic is beta.", true, true);
    if (p_alpha.empty() || p_beta.empty())
    ...
    try {
        ...
        request_result rA = eng.submit_request(build(1, p_alpha, "A")).result.get();
        request_result rb = eng.submit_request(build(2, p_beta, "")).result.get();
        request_result rB = eng.submit_request(build(3, p_beta, "B")).result.get();
        ...
        // No cross-session reuse of any kind for session B.
        if (er.lcp_reuse_admitted_count != 0) {
            char b[160]; std::snprintf(b, sizeof(b),
                "lcp_reuse_admitted_count=%d (expected 0; cross-session must not reuse)",
                er.lcp_reuse_admitted_count);
            return fail_and_cleanup(b);
        }
        if (er.session_reuse_admitted_count != 0)
            return fail_and_cleanup("session_reuse_admitted_count != 0 (cross-session must not reuse)");
        if (rB.n_decoded != rb.n_decoded)
            return fail_and_cleanup("rB n_decoded != baseline n_decoded");
        if (rB.hash != rb.hash) {
            char b[256]; std::snprintf(b, sizeof(b),
                "rB hash=0x%016llx != baseline hash=0x%016llx",
        ...
        fprintf(stdout,
            "baseline hash=0x%016llx rB hash=0x%016llx "
        ...
        fprintf(stdout, "HPX_SESSION_CROSS_SESSION_NO_LCP_SMOKE: ALL_COMPLETED\n");
        fflush(stdout);
