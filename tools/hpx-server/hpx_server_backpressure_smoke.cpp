// hpx_server_backpressure_smoke.cpp — M7e in-process backpressure
// smoke for llama-hpx-server.
//
// Boots model + engine + cpp-httplib server in-process with strict
// capacity (n_seq_max=1, max_concurrent=1), drives a long-running
// "holder" POST that takes the single slot, then sends a "challenger"
// POST that must be rejected with HTTP 503 / `server_busy`. After
// both complete and the lease has drained, sends a serial budget=8
// POST and asserts the canonical greedy hash, proving (a) the door
// rejected the concurrent request, (b) the holder completed normally,
// (c) the lease released, and (d) the server still serves the
// canonical greedy decode correctly.
//
// Determinism anchor:
//   The smoke observes `cap.in_flight` directly (the in-process
//   capacity counter is in the same TU as the inlined handler) and
//   only sends the challenger AFTER the holder has crossed
//   try_acquire. The challenger therefore cannot win the race —
//   try_acquire is guaranteed to fail. No `sleep_for`; the wait loop
//   yields via `hpx::this_thread::yield`. A 30s
//   `std::chrono::steady_clock` deadline guards against pathology
//   (e.g. the holder POST never reaching the server) so the test
...
//   uses `hpx::this_thread::yield`. The in-flight capacity counter
//   is a `std::atomic<int32_t>` at the adapter boundary, following
//   the same precedent as `next_rid` and the M7e capacity counter in
//   `hpx-server.cpp` itself. The handler is inlined here so the
//   smoke is self-contained AND so the smoke driver can observe
//   `cap` directly for its determinism anchor.
...
void print_usage(const char * argv0) {
    fprintf(stderr,
        "usage: %s --model <path>\n"
        "  M7e HPX server backpressure smoke.\n",
        argv0);
}
...
void emit_fail(const std::string & reason) {
    fprintf(stdout, "HPX_SERVER_BACKPRESSURE_SMOKE: FAIL: %s\n",
            reason.c_str());
    fflush(stdout);
}

void emit_pass() {
    fprintf(stdout, "HPX_SERVER_BACKPRESSURE_SMOKE: PASS\n");
    fflush(stdout);
}
...
// Inlined so the smoke is self-contained; the smoke ALSO reads
// `cap.in_flight` directly as its determinism anchor, which is why the
// capacity instance lives in this TU rather than being captured
// opaquely from the server binary.
struct capacity {
...
//
// The poll body uses `hpx::this_thread::yield`, which REQUIRES an HPX
// thread context — calling it on the OS main thread aborts with
// `null_thread_id`. To stay HPX-native from any caller, the loop is
// wrapped in `hpx::async(...).get()` so it always runs on an HPX
// worker. The OS main thread (the smoke driver) blocks on the
// future via the same foreign-thread shared-state cv path that the
// M7a/M7b/M7c/M7d smokes use for `h.result.get()`.
...
int main(int argc, char ** argv) {
    ...
    // hold a worker. 4 gives one spare for HPX bookkeeping. The OS
    // main thread (running the smoke driver) is not an HPX worker;
    // the synchronous challenger POST and the wait_for_capacity poll
    // both run on OS main.
    ...
    try {
        ...
        httplib::Server srv;
        // Handler mirrors the M7e shape of hpx-server.cpp: validation,
        // try_acquire (early 503), then tokenize + submit_request +
        // wait. Streaming branch omitted — backpressure for the SSE
        // lifecycle is deferred to M7e2 per plan.
        srv.Post("/completion",
            ...
            // M7e capacity reservation: after validation (cheap, no
            // engine touch) so malformed requests do not consume a
            // slot, and BEFORE tokenize + submit_request so the
            // engine inbox cannot grow from concurrent clients.
        ...
        // ---- Phase 1: holder occupies the slot ---------------------
        // Launch the holder POST on an HPX worker. budget=64 keeps
        // the engine busy for hundreds of milliseconds — orders of
        // magnitude wider than any try_acquire race window.
        hpx::future<httplib::Result> holder_fut =
            hpx::async([port]() {
                ...
                req_body["decode_budget"] = 64;
                return cli.Post("/completion", req_body.dump(),
                                "application/json");
            });

        // ---- Determinism anchor ------------------------------------
        // Block the smoke driver until cap.in_flight transitions to
        // 1. That happens inside `try_acquire` in the handler, BEFORE
        // tokenize + submit_request. From this point onward the
        // challenger cannot win the race — try_acquire will fail and
        // the handler will emit 503 immediately.
        ...
        // ---- Phase 1 (cont): holder must complete cleanly ----------
        {
            auto resp = holder_fut.get();
            if (!resp) {
                char buf[200];
                std::snprintf(buf, sizeof(buf),
                    "holder POST failed (error=%d)",
                    static_cast<int>(resp.error()));
                return drain_engine_and_fail(buf);
            }
            if (resp->status != 200) {
                char buf[256];
                std::snprintf(buf, sizeof(buf),
                    "holder status=%d (expected 200) body=%s",
                    resp->status, resp->body.c_str());
                return drain_engine_and_fail(buf);
            }
            nlohmann::json out;
            try {
                ...
                return drain_engine_and_fail(
                    std::string("holder body parse failed: ")
                  + e.what());
            }
            if (!out.contains("n_decoded")
             || !out["n_decoded"].is_number_integer()
             || out["n_decoded"].get<int>() != k_holder_budget) {
                char buf[256];
                std::snprintf(buf, sizeof(buf),
                    "holder n_decoded=%s (expected %d) body=%s",
                    out.contains("n_decoded")
                        ? out["n_decoded"].dump().c_str()
                        : "<missing>",
                    k_holder_budget, resp->body.c_str());
                return drain_engine_and_fail(buf);
            }
        }

        // ---- Phase 1 (cont): wait for the lease to release ---------
        // The lease destructor fires when the holder handler returns
        // (after h.result.get()), so this should be near-instant. The
        // 30s deadline is purely defensive.
        if (!wait_for_capacity(cap,
                [](int32_t v) { return v == 0; },
                std::chrono::seconds(30))) {
            return drain_engine_and_fail(
                "capacity never returned to 0 within 30s after holder "
                "completion");
        }

        // ---- Phase 2: serial canonical follow-up -------------------
        // Proves capacity was actually released AND the server still
        // produces a byte-correct canonical greedy decode.
        {
            httplib::Client cli("127.0.0.1", port);
            cli.set_read_timeout(60, 0);
            cli.set_write_timeout(30, 0);
