"""
Phase 1 helper — run exactly one entry of the HPX-vs-std performance matrix
schedule. No matrix driving. No gate enforcement. Summarizers will gate later.

Selection (must pass exactly one of):
  --global-index N
  --cell-id <cell> --layer <layer> --backend <std|hpx> --trial-index N
...
§6.1 / §6.2 gates and records PASS/FAIL.
"""
import argparse
import csv
import hashlib
...
def parse_args():
    p = argparse.ArgumentParser(
        description="run one HPX-vs-std performance-matrix schedule entry"
    )
...
def select_entry(schedule, args):
    ...
    if args.global_index is not None:
        explicit_any = any(
            v is not None
            for v in (args.cell_id, args.layer, args.backend, args.trial_index)
        )
        if explicit_any:
            sys.exit(
                "pass either --global-index OR the explicit "
                "--cell-id/--layer/--backend/--trial-index quad, not both."
            )
        for e in entries:
    ...
    needed = ("cell_id", "layer", "backend", "trial_index")
    vals = (args.cell_id, args.layer, args.backend, args.trial_index)
    if any(v is None for v in vals):
        sys.exit(
            "must pass either --global-index, or all of "
            "--cell-id, --layer, --backend, --trial-index."
        )
    for e in entries:
        if (e["cell_id"], e["layer"], e["backend"], e["trial_index"]) == vals:
            return e
    sys.exit(
...
def main():
    ...
    print(
        f"cell_id={entry['cell_id']}  layer={entry['layer']}  "
        f"backend={entry['backend']}  trial_index={entry['trial_index']}"
    )
