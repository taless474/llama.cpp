"""
Phase 2 helper — summarize one (cell_id, layer, backend) group of trials.
...
Reads:
  ...
  conditions/<cell_id>/<layer_short>/<backend>/trial_NN/{
    bench.stdout, bench.stderr, bench.exit_code.txt,
    per_repeat.csv, hpx_trace.txt, process_wall_ms.txt
  }   for every scheduled trial in the group (expected 31 trials)

Writes (under the same directory):
  conditions/<cell_id>/<layer_short>/<backend>/all_requests.csv
  conditions/<cell_id>/<layer_short>/<backend>/per_trial_summary.csv
  conditions/<cell_id>/<layer_short>/<backend>/condition_summary.txt
...
Layer/backend-specific gates:
  ...
  layer=correctness_trace_on, backend=hpx:
    - exactly one start line  '[serving-bench] hpx_runtime_start_once: starting (os_threads=K)'
    - exactly one ready line  '[serving-bench] engine_hpx ready: n_contexts=N pool_size=N'
    ...
    - if n_contexts > 1: set of acquire ctx ids equals {0..n_contexts-1}
    - total filtered HPX trace lines == expected_trace_lines
...
only and are computed from measured_for_timing=true rows so condition-
level summarizers can reuse them.
"""
import argparse
import csv
import json
...
def parse_args():
    p = argparse.ArgumentParser(description="summarize one (cell, layer, backend) group")
...
def gate_trial(entry, out_dir):
    ...
    # ---- existence of bench.exit_code.txt is itself a hard gate ----
    ...
    # ---- aggregate tokens/sec from stdout ----
    ...
    # ---- process_wall_ms.txt: first line is the float ----
...
def main():
    ...
    entries = [
        ...
        if e["cell_id"] == args.cell_id
        and e["layer"]   == args.layer
        and e["backend"] == args.backend
    ]
    if not entries:
        sys.exit(
            f"no schedule entries match cell_id={args.cell_id} "
    ...
    if len(entries) != expected_trials:
        # not fatal — we just record the count and let the gate fail
        pass

    layer_short = LAYER_DIR[args.layer]
    group_dir = ROOT / "conditions" / args.cell_id / layer_short / args.backend
    ...
    # ---- write all_requests.csv (every trial × every request) ----
    ...
    # ---- descriptive aggregate over measured_for_timing trials only ----
    ...
    # ---- write condition_summary.txt ----
    out = []
    out.append(f"# layer summary: {args.cell_id} / {args.layer} / {args.backend}")
    ...
    out.append("- The condition summarizer (_summarize_condition.py) reads this file's")
    out.append("  machine-readable header and gates the cell's correctness state.")
    out.append("")
    ...
    # ---- console echo ----
    print(f"cell_id={args.cell_id}  layer={args.layer}  backend={args.backend}")
