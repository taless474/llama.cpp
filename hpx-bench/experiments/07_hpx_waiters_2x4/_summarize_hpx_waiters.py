"""
Per-backend summarizer for the (2, 4, 12) HPX waiter-pressure slice.

Variants (single CLI argument):
  python3 _summarize_hpx_waiters.py                  # default: hpx/
  python3 _summarize_hpx_waiters.py hpx              # explicit
  python3 _summarize_hpx_waiters.py std_control      # std-control variant

Reads (under <variant>/):
  per_repeat.csv         (header + 12 data rows; req_index 0..11, kind=request)
...
Common gates (applied in both variants):
  - harness exit code == 0
  - all 12 req lines parsed
  - stdout aggregate line: n_ok=12 n_cancelled=0 n_error=0
  - all 12 status == ok
  - all 12 n_tokens_generated == 16
  - all 12 generated_token_hash equal canonical 0x833045f1e2ebf49f
...
HPX lifecycle / capacity-correctness gates (variant=hpx, positive form):
  ...
  - for each i in 0..11: exactly one acquire and exactly one release
  - for each i in 0..11: acquire ctx id and release ctx id are both in {0, 1}
  - for each i in 0..11: release ctx id equals acquire ctx id (same-ctx pairing)
  - for each i in 0..11: acquire line precedes release line in trace order
  - the set of acquire ctx ids across all 12 requests equals {0, 1}
  - total filtered HPX trace lines == 27   (1 + 1 + 1 + 12 + 12)

These gates jointly assert: HPX came up exactly once with the expected
carrier and pool size, every submitted request reached both an acquire and
a release event (no starvation), the RAII guard kept release paired with
acquire on the same context for every request, and both context slots
stayed reachable across the full 12-request workload. With n_concurrent=4
and n_contexts=2, that means the two excess in-flight requests at
saturation must have been serviced through the waiter path. That is the
no-starvation / capacity-correctness property.
...
Explicitly NOT gated:
  ...
  - FIFO ordering of the waiter queue (no queued/wake trace events emitted today)
  - per-request wait time before acquire (not measured; no queued event to anchor it)
  - whether any two requests overlap in time (no trace timestamps)
  - timing in any form (process_wall_ms, ttft_ms, total_ms, total_ms - ttft_ms)
...
EXPECTED_AGG_LINE         = "n_ok=12 n_cancelled=0 n_error=0"
EXPECTED_TRACE_LINE_COUNT = 27  # 1 start + 1 ready + 1 stop + 12 acquire + 12 release
...
# ---- lifecycle / capacity-correctness gates (variant-specific) ----
...
# ---- descriptive timing over all 12 rows ----
...
# ---- summary text ----
out = []
out.append(f"=== llama-serving-bench {variant} waiter-pressure summary "
           f"(comparison_hpx_waiters/{variant}) ===")
out.append("")
...
out.append("")
out.append("--- caveats ---")
out.append("This is one bench-side leg of the (2, 4, 12) HPX waiter-pressure slice.")
out.append("Cross-backend gates live in comparison_hpx_waiters/comparison_summary.txt.")
out.append("All 12 requests are correctness-gated. No warmup separation.")
...
out.append("with 4 in-flight at saturation) is more severe than the (2, 2) slice and is")
out.append("expected. It will slow each decode but must not change tokens.")
out.append("Trace lines are event-ordered but not timestamped; we do NOT claim true")
out.append("overlap of in-flight requests in time.")
out.append("This slice does NOT establish FIFO waiter ordering. backend_hpx.cpp uses")
out.append("std::deque<hpx::promise> with front-pop (the structural intent is FIFO),")
out.append("but no queued/wake trace events are emitted today, so FIFO is not")
out.append("observable from the trace and is not gated here.")
out.append("This slice does NOT establish per-request wait time before acquire.")
out.append("")
