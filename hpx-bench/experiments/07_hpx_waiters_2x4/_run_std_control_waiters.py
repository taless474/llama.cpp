"""
std-control timing helper for the (2, 4, 12) waiter-pressure slice.
...
LLAMA_SERVING_BENCH_HPX_TRACE=1 still set. Two purposes:
  ...
  - Confirm that compiling LLAMA_SERVING_BENCH_HPX=ON does not change the std
    backend's behavior at the (2, 4, 12) shape (canonical hash on all 12
    requests, n_ok=12, all 16 tokens).
...
Mirrors _run_hpx_waiters.py exactly except:
  - ARGV uses --backend std.
  - ROOT is std_control/.

This script does not gate. It only produces artifacts. The "canonical hash on
all 12 requests AND zero HPX lifecycle trace lines" check is enforced later
in _summarize_hpx_waiters.py (variant std_control) and aggregated in
...
Captures (under std_control/):
  ...
  per_repeat.csv               (header + 12 data rows, with total_minus_ttft_ms)
  request_summary.txt
  binary_used.txt
  build_info.txt
  git_head.txt
...
ROOT   = Path("/Users/Ashk/Desktop/HPX/llama-hpx/local/baselines/comparison_hpx_waiters/std_control")
...
(ROOT / "process_wall_ms.txt").write_text(
    ...
    "scope: (2, 4, 12) std-control of the HPX-on binary at --backend std.\n"
    "Not a gate. The trace env should have no effect at --backend std; that\n"
    "is the structural claim being probed.\n"
)
...
(ROOT / "build_info.txt").write_text(
    ...
    "purpose: std-control of the HPX-on binary at the (2, 4, 12) waiter-pressure shape\n"
)
