"""
Generate the deterministic schedule for the deep-queue short-request
stress experiment. Does not run anything; does not invoke the binary.
...
CELL = {
    ...
    # 1 start + 1 ready + 1 stop + 200 acquire + 200 release
    "expected_trace_lines": 403,
}
...
def write_schedule(entries, path: Path):
    payload = {
        ...
        "cell":               CELL,
        "max_tokens_plan":    list(PLAN),
        "max_tokens_plan_summary": {
            "len":           len(PLAN),
            "min":           min(PLAN),
            "max":           max(PLAN),
            "unique_values": sorted(set(PLAN)),
            "multiset":      {str(k): PLAN.count(k) for k in sorted(set(PLAN))},
        },
        "layers":             LAYERS,
        "backends":           BACKENDS,
...
def write_summary(entries, path: Path):
    ...
    lines = []
    lines.append("Deep-queue short-request — schedule summary")
    lines.append("")
    ...
    lines.append(f"plan unique values:   {sorted(set(PLAN))}")
    lines.append(f"plan-multiset:        {dict({k: PLAN.count(k) for k in sorted(set(PLAN))})}")
