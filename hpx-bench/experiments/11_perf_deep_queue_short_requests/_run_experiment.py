"""
Drive the deep-queue short-request experiment by invoking _run_one_trial.py
in schedule order. Does not summarize; does not gate. Summarizers run
...
Driver logs (per run):
    runs/driver_logs/run-<timestamp>/
        driver_summary.csv   one row per processed entry: gidx, layer,
                             backend, trial, action (RUN|SKIP),
                             helper_rc, wall_ms
        driver.log           full subprocess stdout/stderr per RUN entry

Stop policy:
    Stops on the first nonzero return code from _run_one_trial.py and
    exits with that code. SKIP entries do not affect the stop policy.
"""
import argparse
import json
import subprocess
...
RUN_ONE  = EXP_DIR / "_run_one_trial.py"
LOGS     = RUNS / "driver_logs"


def parse_args():
    p = argparse.ArgumentParser(
        description="run deep-queue short-request schedule entries"
    )
    ...
    p.add_argument("--force", action="store_true",
                   help="re-run entries whose bench.exit_code.txt exists")
    return p.parse_args()
...
def main():
    ...
    summary.write(
        "global_index,layer,backend,trial_index,action,helper_rc,wall_ms\n"
    )
    ...
    for entry in selected:
        ...
        if ec_path.exists() and not args.force:
            summary.write(
                f"{gidx},{entry['layer']},{entry['backend']},"
                f"{entry['trial_index']},SKIP,,0\n"
            )
            summary.flush()
            n_skip += 1
            continue
        ...
        full.write(
            f"\n=== gidx={gidx} layer={entry['layer']} "
            f"backend={entry['backend']} trial={entry['trial_index']} ===\n"
        )
        ...
        summary.write(
            f"{gidx},{entry['layer']},{entry['backend']},"
            f"{entry['trial_index']},RUN,{proc.returncode},{wall_ms:.3f}\n"
        )
        summary.flush()
        n_run += 1
