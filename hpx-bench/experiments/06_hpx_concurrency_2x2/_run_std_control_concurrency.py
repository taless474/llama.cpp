"""
std-control timing helper for the (2,2) concurrency slice.

Runs the SAME HPX-on llama-serving-bench binary with --backend std and
LLAMA_SERVING_BENCH_HPX_TRACE=1 still set. Two purposes:

  - Confirm that compiling LLAMA_SERVING_BENCH_HPX=ON does not change the std
    backend's behavior at the (2,2) shape (canonical hash on all 8 requests,
    n_ok=8, all 16 tokens).
  - Confirm that --backend std emits zero HPX lifecycle/pool trace lines
    even when the trace env var is set, so the trace lines we see in hpx/
    are caused by --backend hpx and nothing else.

Mirrors _run_hpx_concurrency.py exactly except:
  - ARGV uses --backend std.
  - ROOT is std_control/.

This script does not gate. It only produces artifacts. The "canonical hash on
all 8 requests AND zero HPX lifecycle trace lines" check is enforced later in
_summarize_hpx_concurrency.py (variant std_control) and aggregated in
_summarize_concurrency_comparison.py.

Captures (under std_control/):
  bench.stdout
  bench.stderr
  bench.exit_code.txt
  process_wall_ms.txt          (informational, not a gate)
  per_repeat.csv               (header + 8 data rows, with total_minus_ttft_ms)
  request_summary.txt
  binary_used.txt
  build_info.txt
  git_head.txt
...
ROOT   = Path("/Users/Ashk/Desktop/HPX/llama-hpx/local/baselines/comparison_hpx_concurrency/std_control")
...
# ---- run the harness with HPX trace env still enabled (probe) ----
env = os.environ.copy()
...
(ROOT / "process_wall_ms.txt").write_text(
    ...
    "scope: (2,2) std-control of the HPX-on binary at --backend std. Not a gate.\n"
    "the trace env should have no effect at --backend std; that is the\n"
    "structural claim being probed.\n"
)
...
(ROOT / "build_info.txt").write_text(
    ...
    "concurrency shape: --n-contexts 2 --n-concurrent 2 --n-requests 8\n"
    "trace env: LLAMA_SERVING_BENCH_HPX_TRACE=1 (probe; expected ignored at --backend std)\n"
    "purpose: std-control of the HPX-on binary at the (2,2) concurrency shape\n"
)

# ---- console echo ----
print(f"exit_code={exit_code}")
print(f"process_wall_ms={process_wall_ms:.3f}")
print(f"rows_parsed={len(rows)}  (expected 8)")
