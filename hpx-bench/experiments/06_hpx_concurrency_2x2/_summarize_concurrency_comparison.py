"""
Cross-backend summarizer for the (2,2) HPX concurrency slice.
...
Cross-backend gates:
  ...
  - HPX hash equals std_control hash for each req[0..7] (per-request equality)
  - all 16 hashes equal canonical 0x833045f1e2ebf49f
  - all 16 n_tokens_generated == 16
  - HPX lifecycle gates passed (per-line PASS in hpx/summary.txt)

Descriptive timing (NOT gated):
  - HPX (all 8 rows):  ttft_ms / total_ms / total_minus_ttft_ms / tokens_per_second
                       (min, max, mean, median)
  - HPX (req[2..7] trim, presentation only): total_minus_ttft_ms (mean, median)
...
comparison_aligned/). Helpers and summarizers should leave those alone.
"""
import csv
...
TRIM_INDICES        = set(range(2, EXPECTED_N_REQUESTS))   # req[2..7]
...
# ---- summary text ----
out = []
out.append("=== HPX vs std_control concurrency comparison summary "
           "(comparison_hpx_concurrency) ===")
out.append("")
out.append("inputs:")
...
out.append("")
out.append("--- descriptive timing, all 8 rows (NOT gated) ---")
...
out.append("")
out.append("--- descriptive timing, req[2..7] trim, presentation only (NOT gated) ---")
...
out.append("")
out.append("note: this is an n=8 smoke at n_contexts=2, n_concurrent=2.")
out.append("note: oversubscription (2 ctx x 4 threads on a 4-core box) is expected.")
...
out.append("")
out.append("--- caveats ---")
out.append("Cross-backend leg of the (2,2) HPX concurrency slice.")
out.append("Per-backend gates live in <variant>/summary.txt.")
out.append("No HPX speed claim from this n=8 smoke.")
out.append("No claim of true in-flight overlap; trace is event-ordered, not timestamped.")
out.append("This slice does NOT exercise n_concurrent > n_contexts (Gate 9 / a later slice).")
