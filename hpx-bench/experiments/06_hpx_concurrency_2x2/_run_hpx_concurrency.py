"""
HPX-on llama-serving-bench timing helper for the (2,2) concurrency slice.

Mirrors local/baselines/comparison_hpx_vs_std/_run_hpx.py with these changes:
  - ROOT points at the concurrency slice's hpx/ subdirectory.
  - ARGV uses --n-contexts 2 --n-concurrent 2 --n-requests 8.
  - per_repeat.csv has 8 data rows; every row carries kind=request
    (no warmup phase — all 8 requests are correctness-gated).
...
Captures (under hpx/):
  ...
  per_repeat.csv               (header + 8 data rows, with total_minus_ttft_ms column)
  request_summary.txt
  binary_used.txt
  build_info.txt
  git_head.txt
...
ROOT   = Path("/Users/Ashk/Desktop/HPX/llama-hpx/local/baselines/comparison_hpx_concurrency/hpx")
...
(ROOT / "process_wall_ms.txt").write_text(
    ...
    "scope: HPX-on (2,2) concurrency slice. Not a gate.\n"
...
(ROOT / "build_info.txt").write_text(
    ...
    "expected HPX runtime carriers: os_threads = max(1, min(2, 2)) = 2\n"
    "expected pool_size: 2\n"
    "expected total filtered HPX trace lines: 19 = 1 start + 1 ready + 1 stop + 8 acquire + 8 release\n"
)
...
print(f"hpx_trace_lines={len(trace_lines)}  (expected 19)")
