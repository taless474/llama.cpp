"""
Phase 2 helper — drive the C_2x4 n_threads sensitivity sweep by invoking
...
Driver logs:
  local/baselines/perf_thread_sweep/driver_logs/run-<timestamp>/
    driver_summary.csv   one row per processed entry: gidx, n_threads,
                         layer, backend, trial, action (RUN|SKIP),
                         helper_rc, wall_ms
...
def parse_args():
    p = argparse.ArgumentParser(
        description="run C_2x4 n_threads-sweep schedule entries"
    )
...
def main():
    ...
    summary.write(
        "global_index,n_threads,layer,backend,trial_index,action,"
        "helper_rc,wall_ms\n"
    )
    ...
    for entry in selected:
        ...
        if ec_path.exists() and not args.force:
            summary.write(
                f"{gidx},{entry['n_threads']},{entry['layer']},"
        ...
        full.write(f"\n=== gidx={gidx} n_threads={entry['n_threads']} "
