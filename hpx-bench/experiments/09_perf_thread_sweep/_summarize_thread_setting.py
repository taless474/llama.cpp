"""
Phase 2 helper — summarize one n_threads setting across both layers and
both backends, and emit the std-vs-hpx comparison table for that setting.

Usage:
  python3 _summarize_thread_setting.py --n-threads 1

Reads:
  thread_settings/n<N>/correctness/std/condition_summary.txt
  thread_settings/n<N>/correctness/hpx/condition_summary.txt
  thread_settings/n<N>/timing/std/condition_summary.txt
  thread_settings/n<N>/timing/hpx/condition_summary.txt
  thread_settings/n<N>/timing/std/all_requests.csv
  thread_settings/n<N>/timing/hpx/all_requests.csv
  thread_settings/n<N>/timing/std/per_trial_summary.csv
  thread_settings/n<N>/timing/hpx/per_trial_summary.csv
...
Correctness gate (n_threads-level):
...
Timing interpretation uses ONLY rows in:
  thread_settings/n<N>/timing/{std,hpx}/all_requests.csv
where measured_for_timing == true (trial_index in 1..10).

Compared metrics (std vs hpx):
  - total_ms                  pool view (n = n_requests * 10)
  - ttft_ms                   pool view
  - total_minus_ttft_ms       pool view
  - tokens_per_second         pool view
  - process_wall_ms           per-trial view (n = 10)
  - agg_tok_s                 per-trial view (n = 10)
  - median_total_ms           per-trial-median view (n = 10)
  - median_total_minus_ttft   per-trial-median view (n = 10)

For each, report mean, median, p95, p99, stdev_pop, CV (per backend),
and delta (hpx - std), delta % (delta / std median * 100) on the median.

Writes:
  thread_settings/n<N>/condition_comparison.txt

condition_comparison.txt opens with a machine-readable `KEY: VALUE`
block (terminated by `# END HEADER`) so the sweep summarizer can parse
...
def parse_args():
    p = argparse.ArgumentParser(description="summarize one n_threads setting")
...
def main():
    args = parse_args()
    setting_dir = ROOT / "thread_settings" / f"n{args.n_threads}"

    schedule = json.loads(SCHEDULE.read_text())
    sample = next(
        (e for e in schedule["schedule"] if e["n_threads"] == args.n_threads),
        None,
    )
    if sample is None:
        sys.exit(f"no schedule entries for n_threads={args.n_threads}")
    ...
    for layer, ldir in LAYER_DIR.items():
        for backend in ("std", "hpx"):
            path = setting_dir / ldir / backend / "condition_summary.txt"
    ...
    timing_dir_std = setting_dir / "timing" / "std"
    timing_dir_hpx = setting_dir / "timing" / "hpx"
    ...
    out = []
    out.append(f"# thread-setting comparison: n_threads={args.n_threads}")
    ...
    out.append(f"THREAD_OVERALL_CORRECTNESS: {'PASS' if correctness_overall else 'FAIL'}")
    ...
    if not correctness_overall:
        out.append("THREAD_OVERALL_CORRECTNESS: FAIL")
        ...
        out.append("THREAD_OVERALL_CORRECTNESS: PASS")
    ...
    out.append("- Per-trial view: small n (=10 measured) but iid across trials.")
    ...
    out.append("- HPX runtime carriers (os_threads, pool_size) are constant at 2 across the")
    out.append("  sweep; n_threads varies the llama kernel-thread count only.")
    out.append("")

    cmp_path = setting_dir / "condition_comparison.txt"
    setting_dir.mkdir(parents=True, exist_ok=True)
    cmp_path.write_text("\n".join(out))

    print(f"n_threads={args.n_threads}  cell_id={cell_id}")
    ...
    print(f"THREAD_OVERALL_CORRECTNESS: {'PASS' if correctness_overall else 'FAIL'}")
