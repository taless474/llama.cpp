"""
Per-binary llama-serving-bench std summarizer for the aligned comparison run.

Identical in behavior to local/baselines/serving_bench_std_timing/_summarize_timing.py.
Only the ROOT path differs. The CSV in this directory carries one extra
column (total_minus_ttft_ms); csv.DictReader handles it transparently and
it is not consulted by these per-binary gates. The cross-binary
_summarize_comparison.py is the one that reads it.
...
Stats computed over measured rows only (req[1..5]):
  - ttft_ms, total_ms, tokens_per_second   (min, max, mean, median, stdev_pop)

Gates (mirroring serving_bench_std_timing's eleven gates verbatim):
  - harness exit code == 0
...
# ---- summary text ----
out = []
out.append("=== Aligned llama-serving-bench std timing summary (comparison_aligned/bench) ===")
out.append("")
...
out.append("")
out.append("--- caveats ---")
out.append("This is the bench-side leg of the aligned comparison run.")
out.append("Cross-binary gates live in comparison_aligned/comparison_summary.txt.")
out.append("No HPX. No concurrency. Single context, single concurrent.")
out.append("")
