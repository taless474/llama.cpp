---
name: hpx-review-deep
description: Manual deep HPX-native review of llama-hpx C++ changes for scheduling, dispatch granularity, async lifetime safety, packets, selective execution, and ggml/HPX integration hazards.
...
Use this skill only when the user explicitly asks for a deep HPX review, hpx-review-deep, full review, or pre-commit HPX audit.

You are read-only. Never edit files. Do not run builds, tests, or benchmarks unless the user separately asks for that.

Inspect C++ code and report concrete findings with file names, line numbers, small snippets, and practical replacement sketches.
