// test_hpx_packet_mlp_gate_up_f32.cpp
//
// Packet correctness test for GGML_HPX_PACKET_SUBLAYER_MLP_GATE_UP_F32.
//
// Builds the 4-op ggml subgraph via ggml op builders, composes it into a
// 4-region group, compiles a frozen packet, binds one invocation, runs it,
// and checks the output against a scalar reference.
...
TEST(PacketMlpGateUpF32, CompileBindRunMatchesScalarReference)
{
    ...
    ggml_tensor * out      = ggml_mul(gc.ctx, gate_act, up);
    ...
    // Two non-trivial rows to exercise row handling.
    std::vector<float> xb = {
        1.f, 1.f, 1.f, 1.f,
        1.f, 2.f, 3.f, 4.f,
    };
    ...
    std::vector<float> gate_act_buf(rows * out_cols, 0.0f);
    ...
    ASSERT_TRUE(ggml_hpx_compose_mlp_gate_up_group(gate, up, gate_act, out, &grp));
    ...
    // ── Resources ─────────────────────────────────────────────────────────
    // MLP_GATE_UP has no REDUCTION regions; all resource requirements are 0
    // except n_lanes.
...
TEST(PacketMlpGateUpF32, CompileRejectsWrongKeyShape)
{
    constexpr int64_t cols     = 4;
    constexpr int64_t out_cols = 3;
    constexpr int64_t rows     = 2;
    ...
    std::vector<float> gate_act_buf(rows * out_cols, 0.f);
...
//   (b) valid MLP group but key.sublayer is RMS_NORM_F32 (sublayer mismatch)
TEST(PacketMlpGateUpF32, CompileRejectsMalformedGroup)
{
    constexpr int64_t cols     = 4;
    constexpr int64_t out_cols = 3;
    constexpr int64_t rows     = 2;
    ...
    ggml_hpx_mlp_gate_up_group valid_grp;
    ggml_hpx_mlp_gate_up_group_init(&valid_grp);
    ASSERT_TRUE(
        ggml_hpx_compose_mlp_gate_up_group(gate, up, gate_act, out, &valid_grp));
    ...
    // (b) valid MLP group but key says RMS_NORM_F32 — sublayer mismatch
    {
        ggml_hpx_packet_plan_key key{};
        key.sublayer       = GGML_HPX_PACKET_SUBLAYER_RMS_NORM_F32;  // wrong sublayer
        key.team           = GGML_HPX_PACKET_TEAM_DECODE;
        key.n_lanes        = 1;
        key.dtype          = GGML_TYPE_F32;
        key.policy_version = 1;
        key.shape[0]       = out_cols * rows;   // RMS_NORM shape[0] == n
        key.shape[1]       = 0;
        key.shape[2]       = 0;
        key.shape[3]       = 0;
        ...
        EXPECT_EQ(p, nullptr)   << "compile must reject MLP group under RMS_NORM key";
        EXPECT_NE(err, nullptr) << "must set out_err on rejection";
    }
}
